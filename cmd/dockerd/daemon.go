...
func newRouterOptions(config *config.Config, daemon *daemon.Daemon) (routerOptions, error) {
	...
	manager, err := dockerfile.NewBuildManager(daemon, sm, fsCache)
	if err != nil {
		return opts, err
	}

	bb, err := buildbackend.NewBackend(daemon, manager, fsCache)
	if err != nil {
...
func initRouter(opts routerOptions) {
	...
	routers := []router.Router{
		...
		systemrouter.NewRouter(opts.daemon, opts.cluster, opts.fsCache),
