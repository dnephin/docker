...
func ReplaceFileTarWrapper(inputTarStream io.ReadCloser, mods map[string]TarModifierFunc) io.ReadCloser {
	...
	go func() {
		...
		modify := func(name string, original *tar.Header, modifier TarModifierFunc) error {
		...
		for {
			...
			if err := modify(originalHeader.Name, originalHeader, modifier); err != nil {
				pipeWriter.CloseWithError(err)
				return
			}
		}
		...
		for name, modifier := range mods {
			if err := modify(name, nil, modifier); err != nil {
				pipeWriter.CloseWithError(err)
				return
			}
		}
